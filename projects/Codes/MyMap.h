...
private:
  TreeNode<K, V> *global_root = nullptr;

  // Returns reference to the value of a node with key `access_key`
  // If no such node exists, throws std::out_of_range exception
  V &at_helper(TreeNode<K, V> *&root, const K &access_key);

  // Counts the number of nodes in the tree
  int size_helper(TreeNode<K, V> *root) const;

  // De-allocates all nodes in the tree
  void clear_helper(TreeNode<K, V> *&root);

  // Inseroots node with key `new_key` and value `new_value`
  // Does nothing if a node with `new_key` already exists in tree
  // Search property should be maintained
  void insert_helper(TreeNode<K, V> *&root, const K &new_key,
                     const V &new_value);

  // Returns pointer to node with the minimum key
  TreeNode<K, V> *get_min(TreeNode<K, V> *root);

  // Removes node with key `erase_key` if it exists
  // Does nothing otherwise
  // Search property should be maintained
  void erase_helper(TreeNode<K, V> *&root, const K &erase_key);

  // Returns pointer to node with key `search_key` if it exists
  // Returns nullptr otherwise
  TreeNode<K, V> *find_helper(TreeNode<K, V> *root, const K &search_key) const;

  // Prints the elements of the tree with tree-like formatting
  void print_helper(TreeNode<K, V> *root, std::string indent) const;
  ...
  // is valid)
  V &bracket_helper(TreeNode<K, V> *&root, const K &access_key);
  ...
  // children
  TreeNode<K, V> *clone(const TreeNode<K, V> *root);
...
public:
  ...
  void convert_to_sorted_list(TreeNode<K, V> *&root, std::vector<V> &sorted);
};
