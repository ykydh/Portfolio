...
<p>
The system continuously samples an <strong>LSM6DS3 IMU</strong> over SPI, processes sensor data through independent RTOS threads, and transmits telemetry to a host computer for monitoring. The architecture is designed around separated acquisition and processing stages so that sensor sampling can continue independently of higher-level processing and telemetry tasks.
</p>
<p><strong>Highlights</strong></p>
<ul>
  ...
  <li>Designed a thread-based producer-consumer architecture separating sensor sampling from data processing</li>
  <li>Implemented a ring buffer for transferring timestamped IMU samples safely between RTOS threads</li>
  <li>Used RTOS synchronization primitives to protect shared data structures and coordinate concurrent processing</li>
  <li>Converted raw accelerometer and gyroscope measurements into engineering units for downstream processing</li>
  <li>Implemented low-rate telemetry output for host-side monitoring and visualization</li>
  <li>Designed the architecture with fault monitoring and watchdog-based recovery concepts for improved system reliability</li>
</ul>
<p><strong>Technologies</strong></p>
<p>
...
<p align="center">
  <em>AeroCore embedded flight avionics architecture</em>
</p>
</td>
</tr>
</table>
...
The system separates high-frequency sensor acquisition from lower-frequency telemetry and monitoring. The sampling thread prioritizes consistent IMU acquisition, while the processing stage consumes samples from the ring buffer and prepares data for telemetry.

This architecture provides a foundation for adding additional avionics-style reliability features such as watchdog monitoring, fault detection, sensor validation, and safe-state handling.

Engineering Challenges

A major challenge was designing the communication between concurrent RTOS threads while maintaining deterministic sensor acquisition. A ring buffer was used to decouple the producer and consumer threads and prevent the processing stage from directly controlling the timing of sensor sampling.

The project also required configuring Zephyr devicetree and SPI peripherals, interpreting IMU register-level communication, and debugging embedded software across hardware and RTOS layers.
